...
impl Resolver {
    ...
    /// to the root piece.
    fn explode_flat_recipe(recipe: Recipe) -> HashMap<i64, Recipe> {
    ...
    pub async fn generate_bom_entry(
        ...
        //NOTE: probably not necessary to explode the recipe.
        //      Should the need arise, maybe a graph library would be better
        //      for this task.
        let recipe_map = Resolver::explode_flat_recipe(recipe);
...
mod tests {
    ...
    fn test_explode_recipe() {
        ...
        let exploded = Resolver::explode_flat_recipe(recipe);
